...
欢迎来到 **Oh My Skill**。这是一个为 Agent 设计的 Skill 集合，把我自己可能平常的一些嵌入式开发的工作流整理成了可重复执行的提示词。
...
专为启是科技的 MSPM0G3519 板子量身定制的固件开发 Skill。它深度融合了 TI SysConfig 和 Keil 工具链，强制执行严格的引脚分配校验，并能处理特定的硬件校准（如 VREF 实测值），通过三级代码质量检查机制保障代码的健壮性。
👉 [详细介绍及使用方法](./mspm0g3519/README.md)

## 🚀 如何使用

1. 克隆或下载本仓库中你需要的 Skill 文件夹。
2. 将对应的文件夹部署到你的 Agent 或任何支持 `.xxx/skills` 结构的 Agent 环境目录中，或者直接把仓库地址告诉你的 Agent ，让它帮你部署。
