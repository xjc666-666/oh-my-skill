 ...
 * --/COPYRIGHT--*/
/*!****************************************************************************
 *  @file       hallEstimator.h
 *  @brief      HallEstimator Module
 *
 * 
 *  @anchor hallEstimator_h
 *  # Overview
 *
 *
 *  <hr>
...
#ifndef FOC_INCLUDE_HALL_ESTIMATOR_H_
#define FOC_INCLUDE_HALL_ESTIMATOR_H_
...
/*! @brief Hall SPEED FILTER CONSTANT */
...
/*! @brief Hall Estimator structure */
typedef struct HALL_ESTIMATOR_OBJ_T
{
    /*! Hall Angle in PU */
    int32_t   *pThetaHallIndexPU;
    /*! Pointer reference for Sampling Time */
    const int32_t
        *pDeltaT;
    ...
    /*! Double pointer to the FOC Angle */
...
HALL_ESTIMATOR_OBJ_T;

/*! @brief Hall Invalid Stall  structure */
typedef struct HALL_INVALID_STALL_DETECT_T
{
    /*! Hall Sector change Status */
    bool stallDetectEn;
    /*!  Current Hall Index */
    uint16_t  *pHallIndex;
}HALL_INVALID_STALL_DETECT_T;

/**
 * @brief     Hall Angle Estimator run
 * @param[in] *pHallHandle  Pointer
 */
void hallAngleEstimRun(HALL_ESTIMATOR_OBJ_T *pHallHandle);
/**
 * @brief     Hall Angle Forced open Loop run
 * @param[in] *pHallHandle  Pointer
 * @return    Returns the Hall Flux angle
 */
int32_t hallForcedAngleRun(HALL_ESTIMATOR_OBJ_T *pHallHandle);
/**
 * @brief     Hall Angle Finit
 * @param[in] *pHallHandle  Pointer
 */
void hallAngleInit(HALL_ESTIMATOR_OBJ_T *pHallHandle);
/**
 * @brief     Hall Angle Estim - Sine & Cosine
 * @param[in] *pFOCSinCos  MC Sine Cos Pointer
*  @param[in] *pHallHandle  Hall Input Pointer
 */
void hallSinCosEstim(MC_SINCOS_T *pFOCSinCos ,HALL_ESTIMATOR_OBJ_T *pHallHandle);
/**
 * @brief     Invalid Hall Check
 * @param[in] *pHallSector  Pointer
 * @return    Returns true if invalid Hall state is detected
 */
