...
void openLoopInit(OPEN_LOOP_T *pOpenLoop)
{
    ...
    pOpenLoop->flags.b.transitionEnable = FALSE;
...
void openLoopRun(OPEN_LOOP_T *pOpenLoop)
{
    ...
    {
        pOpenLoop->pSpeedRefSlewRate = &pOpenLoop->speedRefSlewRateDecel;
        pOpenLoop->pAccelRefSlewRate = &pOpenLoop->accelRefSlewRateDecel;
    }
...
void openLoopSetInitialVelocity(OPEN_LOOP_T *pOpenLoop)
{
    ...
    if(pOpenLoop->flags.b.fastStartEnable)
    {
        if(targetVelocity > 0)
        {
            initVelocity = pOpenLoop->initialSpeedReference;
        }
        else
        {
            initVelocity = -pOpenLoop->initialSpeedReference;
