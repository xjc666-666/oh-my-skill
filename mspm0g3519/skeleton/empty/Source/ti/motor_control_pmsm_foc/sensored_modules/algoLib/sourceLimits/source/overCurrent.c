...
{
    /*
    pOverCurrent->flags.b.enable = FALSE;
    pOverCurrent->overCurrentLimit = pOverCurrent->overCurrentLimitSet;

    HAL_DisableWindowComparator();

    HAL_SetOverCurrentLimit(pOverCurrent->overCurrentLimit);
    HAL_DisableWindowComparator();
    */
}
...
{
    /*
   CSA_GAIN_TYPES csaGainGet = *(pOverCurrent->pCSAGainGet);

   if(pOverCurrent->csaGainSet >= csaGainGet)
   {
       pOverCurrent->overCurrentLimit =
               pOverCurrent->overCurrentLimitSet <<
               (pOverCurrent->csaGainSet - csaGainGet);
   }
   else
   {
       pOverCurrent->overCurrentLimit =
               pOverCurrent->overCurrentLimitSet >>
               (csaGainGet - pOverCurrent->csaGainSet);
   }

   if((pOverCurrent->overCurrentLimit >= _IQ(1.0)) ||
           (pOverCurrent->overCurrentLimit < 0))
   {
       HAL_DisableWindowComparator();
   }
   else
